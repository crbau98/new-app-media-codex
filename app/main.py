...
# Serve PWA root assets (sw.js, manifest, icons) from the dist root
if (_FRONTEND_DIST / "icons").exists():
    app.mount(
        "/icons",
        CacheControlStaticFiles(
            directory=str(_FRONTEND_DIST / "icons"),
...
def _serve_dist_file(filename: str, media_type: str) -> Response:
    path = _FRONTEND_DIST / filename
    if path.exists():
        return Response(
...
def serve_sw() -> Response:
    return _serve_dist_file("sw.js", "application/javascript")


@app.get("/manifest.webmanifest")
def serve_manifest() -> Response:
    return _serve_dist_file("manifest.webmanifest", "application/manifest+json")
