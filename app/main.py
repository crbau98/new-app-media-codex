...
async def apply_response_headers(request: Request, call_next):
    ...
    # Log slow API requests (> 2s)
    elapsed_ms = (time.monotonic() - t0) * 1000
    if elapsed_ms > 2000 and path.startswith("/api/"):
        print(f"[slow-request] {path} took {elapsed_ms:.0f}ms")
...
# Module-level shared Session with connection pooling â avoids creating a new
# TCP connection pool per compound lookup.
_compound_session = None

def _get_compound_session():
    global _compound_session
    if _compound_session is None:
        import requests
        _compound_session = requests.Session()
        adapter = requests.adapters.HTTPAdapter(pool_connections=4, pool_maxsize=4, max_retries=2)
        _compound_session.mount("https://", adapter)
        _compound_session.mount("http://", adapter)
    return _compound_session
...
async def compound_detail(name: str, request: Request) -> JSONResponse:
    ...
    def _sync_lookup():
        return lookup_compound(_get_compound_session(), name)
...
# SPA fallback â must be the last route
