"""GZip middleware that never compresses video or audio streams.

Starlette's default :class:`starlette.middleware.gzip.GZipMiddleware` only skips
``text/event-stream`` (plus responses that already have ``Content-Encoding``).
Many stacks strip ``Content-Encoding: identity`` as a no-op, so progressive
``<video>`` responses were still gzipped when the client sent
``Accept-Encoding: gzip``. Browsers do not gunzip media element byte streams,
so playback stalled with a spinner.

Vendored from Starlette 0.46 ``gzip.py`` with an extended content-type exclusion.
"""
...
class IdentityResponder:
    ...
    async def send_with_compression(self, message: Message) -> None:
        ...
        if message_type == "http.response.start":
            ...
            self.content_type_is_excluded = _is_non_compressible_media(ct)
